...
function EditProfilePopup({ isOpen, onClose, onUpdateUser, message }) {
  ...
  useEffect(() => {
    formValidator.resetForm({ userName: currentUser.name, about: currentUser.about }, {}, true);
  }, [isOpen]);
  ...
  return (
    <PopupWithForm title='Edit profile' name='profile' isOpen={isOpen} onClose={onClose}>
      <Form
        name='profile'
